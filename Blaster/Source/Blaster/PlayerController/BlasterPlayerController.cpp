...
void ABlasterPlayerController::ClientElimAnnouncement_Implementation(APlayerState* Attacker, APlayerState* Victim)
{
	...
	if (Attacker && Victim && Self)
	{
		...
		if (BlasterHUD)
		{
			if (Attacker == Self && Self != Victim)
			{
				BlasterHUD->AddElimAnnouncement("You", Victim->GetPlayerName());
				return;
			}
			if (Victim == Self && Self != Attacker)
			{
			...
			if (Attacker == Victim && Attacker != Self)
			{
				BlasterHUD->AddElimAnnouncement("You", "themselves");
				return;
			}
			...
			if (PlayerState->GetCompressedPing() * 4 > HighPingThreshould)
			{
				...
				if (BlasterCharacter)
				{
					BlasterCharacter->GetWorldTimerManager().SetTimer(
						PingAnimTimer,
						this,
						&ABlasterPlayerController::StopHighPingWarning,
						HighPingDuration
						);	
				}
			}
...
void ABlasterPlayerController::HandleCooldown()
{
	...
	if (BlasterHUD)
	{
		...
		if (bHUDValid)
		{
			...
			FString AnnoucementText = Announcement::NewMatchStartsIn;
...
{
	ABlasterPlayerState* BlasterPlayerState =  GetPlayerState<ABlasterPlayerState>();
	if (BlasterPlayerState == nullptr)  FString();

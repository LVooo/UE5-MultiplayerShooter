...
AMenuSystemCharacter::AMenuSystemCharacter():
	// 初始化挂在character身上的委托变量，使用FOnCreateSessionCompleteDelegate，并传入回调函数
...
void AMenuSystemCharacter::CreateGameSession()
{
	...
	// 将character身上的委托变量加入interface的委托列表中
	OnlineSessionInterface->AddOnCreateSessionCompleteDelegate_Handle(CreateSessionCompleteDelegate);

	// 创建session时所需的参数
	TSharedPtr<FOnlineSessionSettings> SessionSettings = MakeShareable(new FOnlineSessionSettings());
	SessionSettings->bIsLANMatch = false; // 使本地局域网连接失效
	SessionSettings->NumPublicConnections = 4; // 总共四人在一个session中
	SessionSettings->bAllowJoinInProgress = true; // session运行中允许人中途加入
	SessionSettings->bAllowJoinViaPresence = true; // 允许所在地区连接
	SessionSettings->bShouldAdvertise = true; // 允许steam广播分发session，使人们能找到session
	SessionSettings->bUsesPresence = true; // 允许找到所在地区连接
	SessionSettings->bUseLobbiesIfAvailable = true; // 防止session创建失败
...
void AMenuSystemCharacter::JoinGameSession()
{
	...
	SessionSearch->MaxSearchResults = 10000; // 因为有很多steam游戏使用480这个id，所以尽可能把数字设大
